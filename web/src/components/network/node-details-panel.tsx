...
const DEGREE_LABELS = {
  0: 'Você',
  1: '1º Nível - Conexão Direta',
  2: '2º Nível - Conexão Indireta',
}
...
export function NodeDetailsPanel({ node, onClose }: NodeDetailsPanelProps) {
  ...
  const degreeLabel = DEGREE_LABELS[node.degree as keyof typeof DEGREE_LABELS] || `${node.degree}º Nível`
        ...
        {node.type === 'user' && node.degree === 0 && (
          <div className="mt-6 rounded-lg bg-green-50 p-3">
            <p className="text-sm text-green-800">
              Este é você! O centro da sua rede de conexões.
            </p>
          </div>
        )}
      </div>
